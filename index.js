    ...
    const mpReq = https.request(options, (mpRes) => {
      ...
      mpRes.on('end', () => {
        ...
        if (result.sandbox_init_point || result.init_point) {
          res.json({ sandbox_init_point: result.sandbox_init_point, init_point: result.init_point })
        } else {
          // Mercado Pago recusou - repassa o motivo real em vez de esconder
          console.error('[MercadoPago] Erro:', result)
          res.status(400).json({ erro: result.message || 'Mercado Pago recusou o pagamento', detalhes: result })
        }
      })
    })
