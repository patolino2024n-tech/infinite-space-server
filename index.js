...
app.post('/criar-pagamento', async (req, res) => {
  try {
    ...
    const mpReq = https.request(options, (mpRes) => { let data = ''; mpRes.on('data', chunk => data += chunk); mpRes.on('end', () => { const result = JSON.parse(data); res.json({ sandbox_init_point: result.sandbox_init_point, init_point: result.init_point }) }) })
